...
export default function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    if (name.trim().length < 3) {
      return handleError("Full name must be at least 3 characters");
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

    if (!emailRegex.test(email)) {
      return handleError("Enter valid email address");
    }
    ...
    if (!termsAccepted) {
      return handleError("Please accept terms and conditions");
    }
